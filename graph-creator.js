document.onload = (function(d3, saveAs, Blob, undefined){
  ...
  GraphCreator.prototype.circleMouseDown = function(d3node, d){
    ...
    var d3Details = d3.select("#node-details").html("");
    d3Details.append('h1').text(d.title);
    d3Details.append('em').text(d.tag);
    d3Details.append('p').text('ES ' + d.early_start_date.toDateString());
    d3Details.append('p').text('EF ' + d.early_finish_date.toDateString());
    d3Details.append('p').text('LS ' + d.late_start_date.toDateString());
    d3Details.append('p').text('LF ' + d.late_finish_date.toDateString());
    ...
    d3Details.append('input').attr('value', d.tag).attr('type', 'text').on('change', function(){
      for (var i in thisGraph.nodes) {
        if (thisGraph.nodes[i].id == d.id) {
          thisGraph.nodes[i].tag = this.value;
        }
      }
    });
  ...
  GraphCreator.prototype.updateWindow = function(svg){
    ...
    var y = settings.graphHeigh || window.innerHeight|| docEl.clientHeight|| bodyEl.clientHeight;
  ...
  var width = window.innerWidth || docEl.clientWidth || bodyEl.clientWidth,
      height = GraphCreator.prototype.consts.height || window.innerHeight|| docEl.clientHeight|| bodyEl.clientHeight;
  ...
  var svg = d3.select(settings.appendElSpec).append("svg")
        .attr("width", width)
        .attr("height", settings.graphHeigh);
